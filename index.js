...
//         realTimePrices.gemini.geminiBTC_USD.price = Number(data.events[0].price);

//         if(realTimePrices.gemini.geminiBTC_USD.highest_bid_price > 0){
//           if(parseFloat(data.events[0].price) > realTimePrices.gemini.geminiBTC_USD.highest_bid_price){
//             realTimePrices.gemini.geminiBTC_USD.highest_bid_price = parseFloat(data.events[0].price);
//           }
//         } else {
//           realTimePrices.gemini.geminiBTC_USD.highest_bid_price = parseFloat(data.events[0].price);
//         }

//         if(realTimePrices.gemini.geminiBTC_USD.lowest_ask_price > 0){
//           if(parseFloat(data.events[0].price) < realTimePrices.gemini.geminiBTC_USD.lowest_ask_price){
//             realTimePrices.gemini.geminiBTC_USD.lowest_ask_price = parseFloat(data.events[0].price);
//           }
//         } else {
//           realTimePrices.gemini.geminiBTC_USD.lowest_ask_price = parseFloat(data.events[0].price);
//         }
...
//     websocketClient.addMarketMessageListener(data => {
//       if(data.events[0].side === 'ask'){
//         realTimePrices.gemini.geminiETH_USD.price = Number(data.events[0].price);

//         if(realTimePrices.gemini.geminiETH_USD.highest_bid_price > 0){
//           if(parseFloat(data.events[0].price) > realTimePrices.gemini.geminiETH_USD.highest_bid_price){
//             realTimePrices.gemini.geminiETH_USD.highest_bid_price = parseFloat(data.events[0].price);
//           }
//         } else {
//           realTimePrices.gemini.geminiETH_USD.highest_bid_price = parseFloat(data.events[0].price);
//         }

//         if(realTimePrices.gemini.geminiETH_USD.lowest_ask_price > 0){
//           if(parseFloat(data.events[0].price) < realTimePrices.gemini.geminiETH_USD.lowest_ask_price){
//             realTimePrices.gemini.geminiETH_USD.lowest_ask_price = parseFloat(data.events[0].price);
//           }
//         } else {
//           realTimePrices.gemini.geminiETH_USD.lowest_ask_price = parseFloat(data.events[0].price);
//         }
...
// Turn comment on to view price data
// setInterval(()=>{console.log(cryptoSocket.Exchanges)}, 500);

checkArbitrage = () => {
  let mostProfitableSpread = -Infinity;
  let tradeType;
  if (cryptoSocket.Exchanges.gdax.ETHUSD - cryptoSocket.Exchanges.gemini.ETHUSD > 1) {
    if (cryptoSocket.Exchanges.gdax.ETHUSD - cryptoSocket.Exchanges.gemini.ETHUSD > mostProfitableSpread) {
      mostProfitableSpread = cryptoSocket.Exchanges.gdax.ETHUSD - cryptoSocket.Exchanges.gemini.ETHUSD;
      tradeType = "ETH IS WORTH MORE ON GDAX THAN GEMINI, SPREAD: ";
    }
  }

  if (cryptoSocket.Exchanges.gemini.ETHUSD - cryptoSocket.Exchanges.gdax.ETHUSD > 1) {
    if (cryptoSocket.Exchanges.gemini.ETHUSD - cryptoSocket.Exchanges.gdax.ETHUSD > mostProfitableSpread) {
      mostProfitableSpread = cryptoSocket.Exchanges.gemini.ETHUSD - cryptoSocket.Exchanges.gdax.ETHUSD;
      tradeType = "ETH IS WORTH MORE ON GEMINI THAN GDAX, SPREAD: ";
    }
  }

  if (cryptoSocket.Exchanges.gemini.BTCUSD - cryptoSocket.Exchanges.gdax.BTCUSD > 1) {
    if (cryptoSocket.Exchanges.gemini.BTCUSD - cryptoSocket.Exchanges.gdax.BTCUSD > mostProfitableSpread) {
      mostProfitableSpread = cryptoSocket.Exchanges.gemini.BTCUSD - cryptoSocket.Exchanges.gdax.BTCUSD;
      tradeType = "BTC IS WORTH MORE ON GEMINI THAN GDAX, SPREAD: ";
    }
  }

  if (cryptoSocket.Exchanges.gdax.BTCUSD - cryptoSocket.Exchanges.gemini.BTCUSD > 1) {
    if (cryptoSocket.Exchanges.gdax.BTCUSD - cryptoSocket.Exchanges.gemini.BTCUSD > mostProfitableSpread) {
      mostProfitableSpread = cryptoSocket.Exchanges.gdax.BTCUSD - cryptoSocket.Exchanges.gemini.BTCUSD;
      tradeType = "BTC IS WORTH MORE ON GDAX THAN GEMINI, SPREAD: ";
    }
  }
...
setTimeout(loopConditional, 30000);
